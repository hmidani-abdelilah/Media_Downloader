...
class YouTubeDownloaderApp:
    ...
    def __init__(self, root, lang_code="en"):
        ...
        # 
        self.root.after(1000, self.system_tray_icon)
    ...
    def notification(self):
        ...
        if platform.release() == "11":
            ...
            def show():
                ...
                target_dir = os.path.abspath(self.save_dir)  # Ensure an absolute path structure
                buttons=[{'activationType': 'protocol', 'arguments': f'file:///{target_dir}', 'content': 'Open Folder'}]
                ...
                toast(
                    ...
                    audio={'src': 'ms-winsoundevent:Notification.Looping.Alarm', 'loop': 'false'}, # Declares clear string labels
                    on_click=buttons               # Routes user actions back to Python
                )

            show()

        elif system == "Windows":
            from winotify import Notification, audio # Notifications حقيقية ديال Windows pip install winotify
            def show():
                icon_path = os.path.abspath("asset/Icon.ico")
                Toast = Notification(
                    app_id="Media Downloader",
    ...
    # دالة لإنشاء أيقونة في شريط النظام (System Tray) مع قائمة منسدلة تحتوي على خيارات مثل "فتح التطبيق" و "الخروج"، بحيث يمكن للمستخدم الوصول إلى التطبيق بسهولة من شريط المهام حتى عندما يكون التطبيق مصغرًا أو مخفيًا. هذه الدالة غير مكتملة حاليًا وتحتاج إلى تنفيذ فعلي لإنشاء الأيقونة والتعامل مع الأحداث المرتبطة بها.
    def system_tray_icon(self):
        icon_path = self.resource_path(os.path.join("asset", "Icon.png"))
        image = Image.open(icon_path)
        if hasattr(self, "_tray_started"):
            return
    ...
    def on_closing_tray_icon(self):
        """
        Handles exiting the application from the tray icon menu.
        """
        if self.is_downloading:
            msg = CTkMessagebox(title=self.lang.get("exit_prompt", "Exit?"), message=self.lang.get("ask_to_stop_download_on_exit", "A download is in progress. Are you sure you want to exit?"),
                            icon="question", option_1=self.lang.get("cancel", "Cancel"), option_2=self.lang.get("no", "No"), option_3=self.lang.get("yes", "Yes"))
            response = msg.get()
            if response == self.lang.get("yes", "Yes"):
                stop_download()
                if self.current_download_thread and self.current_download_thread.is_alive():
                    self.current_download_thread.join(timeout=2)
                self.root.destroy()
        else:
            self.root.destroy()
