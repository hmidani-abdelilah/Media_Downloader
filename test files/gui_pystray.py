...
class YouTubeDownloaderApp:
    ...
    def __init__(self, root, lang_code="en"):
        ...
        # Bind close window event to minimize to tray instead of quitting
        # ربط حدث إغلاق النافذة بدالة مخصصة لإخفاء التطبيق في العلبة مع استمرار التنزيل
        self.root.protocol("WM_DELETE_WINDOW", self.hide_window_to_tray)
    
    def create_tray_icon(self):
        """إنشاء أيقونة التطبيق في شريط النظام مع قائمة السياق."""
        if self.tray_icon:
            return

        try:
            icon_file = "Icon.ico" if platform.system() == "Windows" else "Icon.png"
            icon_path = self.resource_path(os.path.join("asset", icon_file))
            image = Image.open(icon_path)
        except Exception:
            image = None

        menu = Menu(
            MenuItem(self.lang.get("open", "Open"), self.show_app),
            MenuItem(self.lang.get("stop_download", "Stop Download"), self.tray_stop_download),
            MenuItem(self.lang.get("exit", "Exit"), self.exit_app)
        )

        self.tray_icon = Icon("Media Downloader", image, "Media Downloader", menu)
        self.tray_thread = threading.Thread(target=self.tray_icon.run, daemon=True)
        self.tray_thread.start()

    def hide_window_to_tray(self):
        """إخفاء النافذة الرئيسية إلى شريط النظام دون إيقاف التنزيلات."""
        try:
    ...
    def show_app(self, icon=None, item=None):
        """عرض نافذة التطبيق من شريط النظام."""
        def _show():
            try:
                self.root.deiconify()
                self.root.lift()
    ...
    def tray_stop_download(self, icon=None, item=None):
        """إيقاف التحميل من خلال قائمة شريط النظام."""
        def _stop():
            if self.is_downloading:
                stop_download()
